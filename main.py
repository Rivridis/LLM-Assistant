...
# Chat Function
def chat(message,history,file_path,chatHist):
    if file_path != None:
        fl = ((Path(file_path).stem).lower()).replace(" ", "_")
        pglst = []
...
with gr.Blocks() as c1:
    ...
    title="AI Assistant",
    clear_btn="Clear",
    additional_inputs=[file_up,(gr.File(render=False,file_types= [".txt"]))],)
